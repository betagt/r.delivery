...
@section('breadcrumbs')
    {!! Breadcrumbs::render('admin_cupons') !!}
@endsection
@section('header')
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <a href="{{ route('admin.cupons.create') }}" class="btn btn-primary btn-flat" target="_blank">
    ...
    <table class="table table-hover">
        ...
        @foreach($list as $item)
            <tr>
                ...
                <td>{{$item->value}}</td>
                <td>
                    <a href="{{route('admin.cupons.edit',['id'=>$item->id])}}" class="btn btn-default"
                       data-toggle="tooltip" data-placement="top" target="_blank" title="Editar #{{ $item->id }}"
                    >
                        <i class="fa fa-pencil"></i>
                    </a>
                    <a href="{{route('admin.cupons.show',['id'=>$item->id])}}" class="btn btn-default"
                       data-toggle="tooltip" data-placement="top" target="_blank" title="Visualizar #{{ $item->id }}"
                    >
                        <i class="fa fa-search"></i>
                    </a>
                    <a href="{{ route('admin.cupons.destroy', [ 'id' => $item->id]) }}" class="btn btn-danger delete"
